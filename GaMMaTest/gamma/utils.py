...
def association(picks, stations, config, event_idx0=0, method="BGMM", pbar=None,):
    ...
    for k in unique_labels:
        ...
        for i in range(len(centers_init)):
            ...
            #I uncommented out the covariance column below, for some reason it was commented out (this should fix the problem)
            event = {
            ...
            for j, k in enumerate(config["dims"]):  ## add location
                ...
                # print("Event location values")
                # print(event[k]) #these get the x, y, and location values
            events.append(event)
